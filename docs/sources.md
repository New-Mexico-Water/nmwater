...
| usace_cwms | API, date-filtered; tables and pool levels are snapshots | `begin`/`end` | manual fetch catches up from the last window; `update` honours since `usace_cwms.py:178-183` | office, series id; `page-size=-1`; windows halve on timeout | none | cheap |
